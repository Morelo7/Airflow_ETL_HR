CREATE OR REPLACE PROCEDURE STG.SP_STG_INSERT_ETL_DEPENDENCIES(V_PROC_NAME IN VARCHAR2)
AS
  CURSOR C1 IS
    SELECT AD.OWNER,
           AD.NAME,
...
BEGIN
  ...
  DELETE STG.TBL_ETL_DEPENDENCIES T
  WHERE T.NAME = UPPER(V_PROC_NAME);

  -- Insert new dependencies
  FOR REC IN C1 LOOP
    BEGIN
      INSERT INTO STG.TBL_ETL_DEPENDENCIES(OWNER,
                                       NAME,
                                       REFERENCED_OWNER,
                                       REFERENCED_NAME,
                                       INSERT_DATE)
...
END SP_STG_INSERT_ETL_DEPENDENCIES;
